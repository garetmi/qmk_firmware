...
static void render_status(void) {
    ...
    switch (get_highest_layer(layer_state)) {
        case _VIA1:
            oled_write_P(PSTR("VERTICAL  "), false);
            break;
        case _VIA2:
            oled_write_P(PSTR("LAYER ONE "), false);
            break;
        case _VIA3:
            oled_write_P(PSTR("LAYER TWO "), false);
            break;
        default: // use BASE case as default
            oled_write_P(PSTR("HORIZONTAL"), false);
    }
    ...
    // Bottom Note
    oled_set_cursor(1, 3);
    oled_write_P(PSTR("2022 xu wei edition"), false);
}
